      ...
      {loading ? (
        ...
        filteredData.length > 0 ? (
          filteredData.map((user) => (
            <div
              key={user._id}
              className='bg-white w-4/5 md:w-3/4 h-auto m-3 p-5 rounded-lg flex flex-col md:flex-row items-center cursor-pointer hover:shadow-lg hover:shadow-blue-200 transition duration-200 ease-in-out'
            >
             ...
             <div className='m-5 w-full md:w-2/5 cursor-pointer justify-center md:justify-start text-blue-700 p-2 rounded-lg'>
             <p className='text-sm'>Skills:</p>
             <ul className='flex flex-wrap  '>
                {user.skills.map((skill, index) => (
                  <li
                    key={index}
                    className='font-semibold  px-2 py-1 rounded-lg m-1 text-sm md:text-base bg-blue-100'
                  >
                    {skill}
                  </li>
                ))}
